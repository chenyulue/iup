...
int str_find(const char* str, const char* str_to_find, int casesensitive) {
    ...
    int str_to_find_len = (int)strlen(str_to_find);
    int count = str_len - str_to_find_len;
...
int item_exit_action_cb(void) { return IUP_CLOSE; }
...
int item_goto_action_cb(Ihandle* item_goto) {
    ...
    txt = IupText(NULL);
    IupSetAttribute(txt, "MASK",
                    IUP_MASK_UINT); /* unsigned integer numbers only */
    ...
    box = IupVbox(lbl, txt,
                  IupSetAttributes(IupHbox(IupFill(), bt_ok, bt_cancel, NULL),
...
int find_next_action_cb(Ihandle* bt_next) {
    ...
    if (pos >= 0)
        ...
        pos = str_find(str, str_to_find,
                       casesensitive); /* try again from the start */

    if (pos >= 0) {
        int lin, col, end_pos = pos + (int)strlen(str_to_find);
        ...
        IupTextConvertLinColToPos(
            multitext, lin, 0, &pos); /* position at col=0, just scroll lines */
        IupSetInt(multitext, "SCROLLTOPOS", pos);
    } else
...
int item_find_action_cb(Ihandle* item_find) {
    ...
    if (!dlg) {
        ...
        box = IupVbox(
            IupLabel("Find What:"), txt, find_case,
            IupSetAttributes(IupHbox(IupFill(), bt_next, bt_close, NULL),
                             "NORMALIZESIZE=HORIZONTAL"),
            NULL);
        IupSetAttribute(box, "MARGIN", "10x10");
        IupSetAttribute(box, "GAP", "5");
...
int main(int argc, char** argv) {
    ...
    edit_menu = IupMenu(item_find, item_goto, NULL);
